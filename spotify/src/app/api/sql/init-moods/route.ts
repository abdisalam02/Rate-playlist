...
export async function GET() {
  try {
    console.log('Initializing mood tables...');
    
    // Enable UUID extension if not already enabled
    const { error: uuidExtError } = await supabase.rpc('execute_sql', {
      sql_query: `CREATE EXTENSION IF NOT EXISTS "uuid-ossp";`
    });
    
    if (uuidExtError) {
      console.error('Error enabling UUID extension:', uuidExtError);
    }
    
    // Create the staple_moods table
    const { error: stapleMoodsError } = await supabase.rpc('execute_sql', {
      sql_query: `
    ...
    if (stapleMoodsError) {
      console.error('Error creating staple_moods table:', stapleMoodsError);
      return NextResponse.json({
        success: false,
        message: 'Failed to create staple_moods table',
        error: stapleMoodsError
      }, { status: 500 });
    }
    
    // Create the user_moods table
    const { error: userMoodsError } = await supabase.rpc('execute_sql', {
      sql_query: `
    ...
    if (userMoodsError) {
      console.error('Error creating user_moods table:', userMoodsError);
      return NextResponse.json({
        success: false,
        message: 'Failed to create user_moods table',
        error: userMoodsError
      }, { status: 500 });
    }
    
    // Create the mood_tracks table
    const { error: moodTracksError } = await supabase.rpc('execute_sql', {
      sql_query: `
    ...
    if (moodTracksError) {
      console.error('Error creating mood_tracks table:', moodTracksError);
      return NextResponse.json({
        success: false,
        message: 'Failed to create mood_tracks table',
        error: moodTracksError
      }, { status: 500 });
    }
    
    // Insert default staple moods if they don't exist
    const { error: insertStapleMoodsError } = await supabase.rpc('execute_sql', {
      sql_query: `
    ...
    if (insertStapleMoodsError) {
      console.error('Error inserting staple moods:', insertStapleMoodsError);
      // Continue anyway
    }
    
    return NextResponse.json({
      success: true,
      message: 'Moods tables created and initialized successfully',
      stapleMoodsResult: stapleMoodsError ? { error: stapleMoodsError.message } : { success: true },
      userMoodsResult: userMoodsError ? { error: userMoodsError.message } : { success: true },
      moodTracksResult: moodTracksError ? { error: moodTracksError.message } : { success: true },
      insertResult: insertStapleMoodsError ? { error: insertStapleMoodsError.message } : { success: true }
    });
    
  } catch (error) {
    console.error('Error in init-moods route:', error);
    return NextResponse.json({
      success: false,
      message: 'Mood tables initialization failed',
