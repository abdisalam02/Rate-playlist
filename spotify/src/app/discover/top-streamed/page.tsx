...
export default function TopStreamedPage() {
  ...
  useEffect(() => {
    const fetchTracks = async () => {
      try {
        ...
        const url = `/api/discover/top-streamed?limit=${pageSize}&use_client_credentials=true`;
        console.log("Fetching top streamed tracks from:", url);
        ...
        if (data.tracks && Array.isArray(data.tracks)) {
          ...
          setSource(data.source || 'Spotify Top Tracks');
          console.log(`Loaded ${data.tracks.length} top streamed tracks successfully`);
        } else if (data.items && Array.isArray(data.items)) {
          setTracks(data.items);
          setTotalCount(data.total || data.items.length);
          setSource(data.source || 'Spotify Top Tracks');
          console.log(`Loaded ${data.items.length} top streamed tracks successfully`);
        } else {
          throw new Error('Invalid data format received');
        }
      } catch (err) {
        console.error('Error fetching top streamed tracks:', err);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#121212] to-[#181818] text-white">
      ...
      <main className="pt-28 pb-20 px-4 max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">All-Time Most Streamed</h1>
          <p className="text-gray-400">
            The most popular tracks of all time on Spotify
          </p>
