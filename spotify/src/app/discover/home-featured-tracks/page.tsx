  ...
  useEffect(() => {
    const fetchTracks = async () => {
      try {
        ...
        const url = `/api/discover/home-featured-tracks?tracks_per_playlist=10&shuffle=true&use_client_credentials=true`;
        ...
        if (data.tracks && Array.isArray(data.tracks)) {
          ...
          // Extract unique playlists from track data
          const playlistNames = new Set<string>();
          data.tracks.forEach((track: Track) => {
            if (track.source_playlist?.name) {
              playlistNames.add(track.source_playlist.name);
            } else if (track.playlist_name) {
              playlistNames.add(track.playlist_name);
            }
          });
          
          setPlaylists(Array.from(playlistNames));
          console.log(`Loaded ${data.tracks.length} tracks from ${playlistNames.size} playlists`);
        } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#121212] to-[#181818] text-white">
      ...
      <main className="pt-28 pb-20 px-4 max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">Featured Playlist Mix</h1>
          <p className="text-gray-400">
            Curated tracks from popular playlists
          </p>
          {playlists.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-2">
              {playlists.map(playlist => (
                <span key={playlist} className="px-3 py-1 bg-[#282828] rounded-full text-xs">
                  {playlist}
                </span>
              ))}
            </div>
          )}
          <p className="text-sm text-green-500 mt-3">{totalCount} tracks total</p>
        </div>
