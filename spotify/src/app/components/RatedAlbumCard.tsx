...
// Album Card Component for Ratings Pages
export default function RatedAlbumCard({ item }: { item: RatingItem & Partial<SpotifyItem> }) {
  const name = item.name || 'Unknown Album';
  const artists = item.artists || [{ name: 'Various Artists' }];
  const imageUrl = item.images?.[0]?.url || '/img/placeholder-album.png'; // Updated placeholder path
  ...
  return (
    <div className="bg-neutral-800/60 rounded-lg overflow-hidden flex flex-col h-full group transition-all duration-300 hover:bg-neutral-700/80 hover:shadow-lg">
       <Link href={spotifyUrl || `/album/${item.id || item.item_id}`} target={spotifyUrl ? "_blank" : undefined} rel={spotifyUrl ? "noopener noreferrer" : undefined} className="block relative">
        <div className="aspect-square relative">
          <Image
            ...
            onError={(e) => { e.currentTarget.src = '/img/placeholder-album.png'; }} // Simple fallback
            loading="lazy"
          />
             ...
             {spotifyUrl && (
                 <span className="p-2 bg-black/50 rounded-full">
                     <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-white" viewBox="0 0 20 20" fill="currentColor">
                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11.414V10a1 1 0 002 0V7a1 1 0 00-1-1h-3a1 1 0 100 2h1.586l-3.293 3.293a1 1 0 101.414 1.414L11 8.414z" clipRule="evenodd" />
                    </svg>
                </span>
             )}
           </div>
      ...
      <div className="p-3 flex-grow flex flex-col justify-between">
        <div>
           <Link href={spotifyUrl || `/album/${item.id || item.item_id}`} target={spotifyUrl ? "_blank" : undefined} rel={spotifyUrl ? "noopener noreferrer" : undefined}>
             <h3 className="font-semibold text-white truncate hover:underline text-sm mb-0.5" title={name}>{name}</h3>
           </Link>
           <p className="text-xs text-neutral-400 truncate" title={artistNames}>
             {releaseYear ? `${releaseYear} • ` : ''}{artistNames}
           </p>
        </div>
